...
class WC_POS_APIv2_Customers extends WC_POS_API_Abstract {
  ...
  private function customer_search($term, $wp_user_query){
    ...
    // var_dump($term);
    // $usermeta_ids =$wpdb->get_col("SELECT DISTINCT user_id
    
    //       FROM $wpdb->usermeta
    
    //       WHERE (meta_key='digits_phone_no' OR meta_key='digits_phone')
    ...
    if (!empty($ids_str)){
      ...
      $wp_user_query->query_where = str_replace(
       ...
       // $wp_user_query->query_where = "WHERE 1=1 AND ID IN(".$ids_str.")";

    }
     // $wp_user_query->query_where = "WHERE 1=1 AND ID IN(".$ids_str.")";
     // var_dump($wp_user_query->query_where);die();
